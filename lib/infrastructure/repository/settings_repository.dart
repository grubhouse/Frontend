...
class SettingsRepository implements SettingsRepositoryFacade {
  ...
  Future<ApiResult<GlobalSettingsResponse>> getGlobalSettings() async {
    try {
      final client = inject<HttpService>().client(requireAuth: false)..options.connectTimeout = const Duration(seconds: 200);
  ...
  Future<ApiResult<GenerateImageModel>> getGenerateImage(String name) async {
    try {
      final client =
          inject<HttpService>().client(chatGpt: true, requireAuth: true)..options.connectTimeout = const Duration(seconds: 30);
